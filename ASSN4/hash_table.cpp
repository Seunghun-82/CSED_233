...
void HashTable::insert(int key) {
    /////////////////////////////////////////////////////////
    //////////  TODO: Implement From Here      //////////////
    int before = hf->hashing(key);
    for (int i = 0; i < table_size; i++)
    {
        if (states[before % table_size + i] == EMPTY || states[before % table_size + i] == DELETED)
        {
            table[(before + i) % table_size] = key;
            states[(before + i) % table_size] = OCCUPIED;
            return;
        }
    }
    ///////////      End of Implementation      /////////////
